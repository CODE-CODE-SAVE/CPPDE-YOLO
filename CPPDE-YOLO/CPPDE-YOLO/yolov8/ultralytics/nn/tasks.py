...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Segment, Detect_dyhead)):
...
class ClassificationModel(BaseModel):
    ...
    def __init__(self, cfg=r'H:\zhaojiefeng\yolov8\yolov8\ultralytics\cfg\models\v8\yolov8s-new-success.yaml', ch=3, nc=None, verbose=True):
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        t = type(m)
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect, Segment, Detect_AFPN4,
                 Detect_AFPN3, Detect_ASFF, Detect_FRM, Detect_dyhead, CLLAHead, Detect_dyhead3, Detect_DySnakeConv,
                 Segment_DySnakeConv, Segment_DBB, Detect_DBB, OBB, Detect_FASFF):
            m.inplace = inplace
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        elif m in { efficientnet_v2, FasterNet,
                    Ghostnetv2}:
            m = m(*args)
...
def guess_model_task(model):
    ...
    def cfg2task(cfg):
        ...
        #     return "classify"
########################################################################################################################
        if m in ("classify", "classifier", "cls", "fc", 'classify_dbb'):
            return "classify"
########################################################################################################################
        if m == "detect":
            return "detect"
        if m in ('segment', 'segment_dysnakeconv', 'segment_dbb'):
            return "segment"
        if m in ('pose', 'pose_dbb'):
            return "pose"
        if m == "obb":
            return "obb"
        else:
