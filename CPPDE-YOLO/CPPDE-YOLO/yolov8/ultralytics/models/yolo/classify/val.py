...
class ClassificationValidator(BaseValidator):
    ...
    def calculate_top1_top5_accuracies(predictions, targets):
        """Calculate top1 and top5 accuracies from predictions and targets."""
        maxk = max((1, 5))
        batch_size = targets.size(0)

        # Get the top5 predictions for each input
        _, pred = predictions.topk(maxk, 1, True, True)
        pred = pred.t()
        correct = pred.eq(targets.view(1, -1).expand_as(pred))

        # Calculate top1 and top5 accuracies
        top1_correct = correct[:1].reshape(-1).float().sum(0, keepdim=True)
        top5_correct = correct[:5].reshape(-1).float().sum(0, keepdim=True)

        top1_accuracy = top1_correct.mul_(100.0 / batch_size)
        top5_accuracy = top5_correct.mul_(100.0 / batch_size)
