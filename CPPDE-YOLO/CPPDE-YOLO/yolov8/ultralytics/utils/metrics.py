...
class ConfusionMatrix:
    ...
    #         self.matrix[p][t] += 1
    def process_cls_preds(self, preds, targets):
        self.names = ['Arts & Photography', 'Biographies & Memoirs', 'Business & Money', 'Calendars',
                      'Children\'s Books', 'Christian Books & Bibles', 'Comics & Graphic Novels',
        ...
        """
        Update confusion matrix and calculate top1, top3, and top5 accuracies for each class in classification task.

        Args:
            preds (Array[N, K]): Predicted class labels, sorted by confidence. K >= 5 for top5 calculation.
            targets (Array[N, 1]): Ground truth class labels.
        """
        preds, targets = torch.cat(preds), torch.cat(targets)
        assert preds.shape[1] >= 5, "Preds must have at least 5 columns for top5 calculation."

        class_correct_top1 = {i: 0 for i in range(len(self.names))}
        class_correct_top3 = {i: 0 for i in range(len(self.names))}
        class_correct_top5 = {i: 0 for i in range(len(self.names))}
        class_counts = {i: 0 for i in range(len(self.names))}
        ...
        for i in range(preds.shape[0]):
            ...
            # 更新混淆矩阵
            self.matrix[int(pred_labels[0])][true_label] += 1
            ...
            if true_label in pred_labels[:3]:
                class_correct_top3[true_label] += 1

            # 更新top5准确率统计
            if true_label in pred_labels[:5]:
                class_correct_top5[true_label] += 1

        # 计算并打印每个类别的top1, top3, 和top5准确率
        for cls in range(len(self.names)):
            if class_counts[cls] > 0:  # 避免除以零
                top1_acc = class_correct_top1[cls] / class_counts[cls]
                top3_acc = class_correct_top3[cls] / class_counts[cls]
                top5_acc = class_correct_top5[cls] / class_counts[cls]
                print(
                    f"{self.names[cls]} - Top1 Accuracy: {top1_acc * 100:.2f}%, Top3 Accuracy: {top3_acc * 100:.2f}%, Top5 Accuracy: {top5_acc * 100:.2f}%")
            else:
                print(f"{self.names[cls]} - No samples found.")
...
class ClassifyMetrics(SimpleClass):
    ...
    def process(self, targets, pred):
        ...
        # 计算概率
        probabilities = torch.softmax(pred.float(), dim=1)
        correct = (targets[:, None] == pred).float()
        acc = torch.stack((correct[:, 0], correct.max(1).values), dim=1)  # (top1, top5) accuracy
        self.top1, self.top5 = acc.mean(0).tolist()

        #保存最后一轮的概率
        self.last_probabilities = probabilities.detach().cpu().numpy()
    # def __init__(self,  num_classes):
    #     self.num_classes = num_classes
    #     self.class_correct_top1 = torch.zeros(num_classes)
    #     self.class_correct_top5 = torch.zeros(num_classes)
    #     self.class_samples = torch.zeros(num_classes)
    #     self.top1 = 0
    #     self.top5 = 0
    #     self.speed = {"preprocess": 0.0, "inference": 0.0, "loss": 0.0, "postprocess": 0.0}
    #     self.task = "classify"
    ...
    # # Top1Top5修改
    # def process(self, targets, preds2):
    #     targets = torch.cat(targets)
    #     print("222222222222-----", targets)
    #     print(targets.shape)
    #     # 确保preds是原始概率分布
    #     preds2 = torch.softmax(torch.cat(preds2).float(), dim=1)  # 将preds2转换为float类型
    #     print("2222222222======", preds2)
    #
    #     # 初始化每个类别的准确率统计
    #     self.class_correct_top1 = torch.zeros(self.num_classes)
    #     self.class_correct_top5 = torch.zeros(self.num_classes)
    #     self.class_samples = torch.zeros(self.num_classes)
    #
    #     # 计算Top-1和Top-5准确率
    #     top1_correct = preds2.argmax(dim=1) == targets
    #     self.top1 = top1_correct.float().mean().item()  # 总的Top-1准确率
    #
    #     _, top5_preds = preds2.topk(5, dim=1, largest=True, sorted=True)
    #     top5_correct = top5_preds.eq(targets.view(-1, 1).expand_as(top5_preds)).sum(dim=1) > 0
    #     self.top5 = top5_correct.float().mean().item()  # 总的Top-5准确率
    #
    #     # 更新每个类别的统计信息
    #     for i in range(targets.size(0)):
    #         label = targets[i].item()
    #         self.class_samples[label] += 1
    #         self.class_correct_top1[label] += top1_correct[i].item()
    #         self.class_correct_top5[label] += top5_correct[i].item()
    #
    #     # 计算每个类别的Top-1和Top-5准确率
    #     self.class_top1_accuracy = self.class_correct_top1 / self.class_samples.clamp(min=1)
    #     self.class_top5_accuracy = self.class_correct_top5 / self.class_samples.clamp(min=1)
    #
    # def class_accuracy(self):
    #     """Calculates class-wise accuracy."""
    #     class_accuracy_top1 = self.class_correct_top1 / self.class_samples.clamp(min=1)  # Avoid division by zero
    #     class_accuracy_top5 = self.class_correct_top5 / self.class_samples.clamp(min=1)
    #     return class_accuracy_top1, class_accuracy_top5

        # 22222
    def print_class_accuracy(self, names):
        top1, top5 = self.class_accuracy()  # 获取每个类别的Top-1和Top-5正确率
        for i, name in enumerate(names):  # 遍历所有类别
            print(f"{name}: Top-1 accuracy = {top1[i] * 100:.2f}%, Top-5 accuracy = {top5[i] * 100:.2f}%")
