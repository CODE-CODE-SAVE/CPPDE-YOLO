...
class v8DetectionLoss:
    ...
    def __init__(self, model):  # model must be de-paralleled
        ...
        # self.bce = QualityfocalLoss()  # 目前支持者目标检测需要注意！
        self.hyp = h
...
#         ##xiugai
#         #print(batch["cls"])
#         batch_size = preds.size(0)  # 获取批次大小
#         num_classes = preds.size(1)  # 获取类别数
#
#         # 初始化全为0的二维张量
#         one_hot_targets = torch.zeros(batch_size, num_classes, device=preds.device)
#
#         # 填充one_hot_targets
#         for i, labels in enumerate(batch["cls"]):
#             one_hot_targets[i, labels] = 1  # 对于每个样本，将对应类别的位置设置为1
#
#         # 使用 SlideLoss
#         slide_loss = SlideLoss(nn.BCEWithLogitsLoss(reduction='none'))
#         loss = slide_loss(preds, one_hot_targets).mean()
#         loss_items = loss.detach()
#         return loss, loss_items
###########################以下是原代码
class v8ClassificationLoss:
